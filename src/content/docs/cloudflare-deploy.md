...
### 连接 GitHub / GitLab 时务必授权仓库

从 **GitHub/GitLab 仓库**部署前，需先在 **个人资料 → 连接** 里连接对应账号：

- **GitHub**：OAuth 授权页会询问你要授权哪些仓库——**请勾选需要部署的仓库**（可"全部仓库"或单选）。仅授权登录（不授权仓库）的话，部署拉取仓库会失败。
- **GitLab**：连接后授权页会列出请求的权限（含仓库读取），确认即可。
- 若之前用旧的（仅登录）授权连接过，**重新连接一次**即可拿到仓库权限；授权界面会重新让你选择仓库。

> 出于安全，服务端**只允许部署你自己已授权账号下的仓库**，无法用它去拉取任意外部仓库。
...
1. 在 **Pages 项目列表**里，找到该项目，点该项目卡片上的 **添加域名**，输入一个域名（如 `app.example.com`）并提交，会自动绑定到该项目对应的 Pages 项目。
2. 去你的域名 DNS 服务商，把该域名解析到 Cloudflare（走 Cloudflare 代理）。
3. 域名生效后，即可通过 `https://app.example.com` 访问该部署。项目卡片上会列出当前已绑定的全部域名（可直接复制）。

> 自定义域名需要你的域名已由 Cloudflare 管理（或解析指向 Cloudflare），Pages 才能签发证书。

## 重新部署

项目详情页（`/pages/[id]`）的 **重新部署** 会按**该次部署原本的来源**跳转到对应的部署界面，而不是跳回通用选择页：

- 工作区部署 → 回到所选工作区的部署界面；
- GitHub/GitLab 部署 → 回到同一个仓库 + 分支；
- ZIP 上传 → 回到上传界面。

> 部署来源会记录在部署记录里（`source` 字段），所以重新部署不会丢失来源。
...
- **仓库归属校验**：Git 部署前会校验目标仓库**必须属于当前已连接账号**，拒绝拉取任意外部仓库，防止 OAuth Token 被滥用（SSRF 类风险）。
- **上传安全**：ZIP 上传会做严格校验——拒绝路径穿越（`..`）、绝对路径、控制字符，并限制文件数量与总大小（≤ 100 MB），防止恶意压缩包逃逸或耗尽内存。
- **路径收口**：所有来源（工作区 / Git / 上传）的文件路径在进入 Cloudflare 前统一校验，含危险片段的文件一律丢弃。
- **环境变量校验**：注入部署文件的 `$KEY` 占位环境变量做了键名/数量/长度限制，防止注入异常内容。
- **权限最小化**：部署使用的 `PAGES_KEY` 建议仅授予 **Cloudflare Pages → Edit/Deploy**，不要使用全权限 Token。
