...
1. 打开 **工作区** 页。
2. 右上角（「新建」旁边）点 **部署**，跳转到独立部署页 `/workspace/deploy`（支持中英文）。
3. 默认选择「从工作区部署」。
4. （可选）在 **构建配置** 里填写安装命令、构建命令、输出目录、环境变量（JSON，支持 `$KEY` 占位注入到文件内容）。**不填也能直接部署**。
5. 点 **部署**，右侧终端控制台会**实时滚动**构建日志。
6. 部署成功后**自动跳转**到部署详情页 `/workspace/deploy/[id]`，展示地址、短链、状态、完整日志，可复制、打开、立即检测、重新部署。

## 使用步骤（ZIP 上传）

1. 在部署页切换到「上传 ZIP 部署」。
2. 点击或拖拽一个 `.zip` 压缩包（内容为要部署的静态文件，如 `index.html`、CSS、JS）。
3. （可选）填写构建配置。
4. 点 **部署 ZIP**，实时观看日志，成功后自动跳转详情页。
