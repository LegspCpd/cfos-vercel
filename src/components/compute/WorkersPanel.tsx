...
export function WorkersPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const [deployKind, setDeployKind] = useState<'worker' | 'pages'>('worker');
  ...
  async function runDeploy() {
    ...
    try {
      ...
      if (r.error) {
        ...
        // Deploy success → go to the worker's DETAIL page (not the editor). The detail page
        // has the "编辑代码" button that enters the fullscreen IDE.
        if (r.deploymentId) {
          setTimeout(() => {
      ...
      {workers.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-lg border bg-card">
          ...
          {workers.map((w, idx) => (
            ...
            >
              <button
                onClick={() => router.push(`/compute/worker/${w.id}/detail`)}
                className="min-w-0 text-left sm:flex-1"
                title={t('wk.ide.back') || 'Open details'}
              >
                <div className="flex items-center gap-1.5">
                  <Code2 className="h-3.5 w-3.5 shrink-0 text-primary" />
                  <span className="truncate text-sm font-semibold hover:underline">{w.projectName || w.workerName}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !deploying && setShowDeploy(false)}>
          <div className="animate-sheet-in flex max-h-[90vh] w-full max-w-2xl flex-col rounded-lg border bg-background p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold">{t('wk.create.title') || 'Create a new project'}</h3>
            ...
            <div className="space-y-3 overflow-y-auto">
              ...
              <div>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <button
                    onClick={() => setDeployKind('worker')}
                    className={clsx(
                      'flex items-start gap-2.5 rounded-lg border p-3 text-left transition-colors',
                      deployKind === 'worker' ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'hover:bg-secondary/50',
                    )}
                  >
                    <Code2 className={clsx('mt-0.5 h-5 w-5 shrink-0', deployKind === 'worker' ? 'text-primary' : 'text-muted-foreground')} />
                    <span>
                  ...
                  <button
                    onClick={() => setDeployKind('pages')}
                    className={clsx(
                      'flex items-start gap-2.5 rounded-lg border p-3 text-left transition-colors',
                      deployKind === 'pages' ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'hover:bg-secondary/50',
                    )}
                  >
                    <LayoutTemplate className={clsx('mt-0.5 h-5 w-5 shrink-0', deployKind === 'pages' ? 'text-primary' : 'text-muted-foreground')} />
                    <span>
