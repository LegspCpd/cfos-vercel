...
export async function GET(req: Request) {
  ...
  // The expensive part (workspace/file counts + audit reads across primary and cold DBs) is
  // cached per-user in KV. The request's OWN IP is always computed live (it varies per
  // request and must not be served from another user's cache entry).
  const data = await cachedJson(
    ...
    { ttlSeconds: Number(process.env.KV_ANALYTICS_TTL) || 30 },
  );
  ...
  return NextResponse.json({ ...data, currentIp, currentIpFamily: ipFamily(currentIp) });
}
