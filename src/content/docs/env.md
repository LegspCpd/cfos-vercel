...
**硬限制（构建时强制）：**
- 项目**最多支持 5 个 Neon 数据库**（1 主库 + 4 副库，`DATABASE_URL_2`..`DATABASE_URL_5`）
- 若构建环境里配置了第 6 个及以上（如 `DATABASE_URL_6`），**构建会直接失败**并提示"此项目只支持最多 5 个 Neon 数据库，请删掉一个"。这是为了防止配置超出上限却静默忽略，导致副库路由错乱

**稳定性保证（多库故障不影响主功能）：**
- 副库连接失败、初始化失败或未建表（schema 缺失）时，**冷数据写入自动回退到主库**（`audit` 已内置回退，`verification` 也做了回退），**主流程（注册、改邮箱、审计）绝不会因副库故障而中断**
- 合并读取对每个副库单独容错：某个副库查询失败只跳过它，不阻断整体结果
