...
- **多人实时协作**（Liveblocks：协作者同屏编辑，内容实时同步、显示在线人数）

**分享与协作**
- 文件分享（Cloudflare R2，带有效期）
- **一键静态发布**（把工作区构建成纯静态站 + 公开链接，无需外部部署）
- 蓝图导出/导入（.gadget.json 归档）
- 公开蓝图分享链接（他人无需登录即可查看）
- 收藏工作区（星标）
- **工作区协作者**（按用户名邀请，只读/可编辑）
- **文件级分享**（单文件授权，不开放整个工作区）
- **公共上下文库**（文档提交 → 管理员审核 → 全员可见，agent 自动参考）
- **站内通知 + 邮件通知**（协作者、审核结果、工单回复等）

**管理与扩展**
- 外部连接（GitHub）
- **SSH 持久会话**（记住服务器目录/环境，跨命令保持）
- **定时任务调度**（cron 表达式，自动执行、日志可查）
- 上下文文档库（agent 参考）
- 管理后台（用户/设置/AI/审计）
- 操作审计日志（可导出 CSV / JSON）
- **AI 用量配额**（按用户/分组每日限额，超限自动熔断）
...
- [一键静态发布](/docs/publish) — 工作区直接发布为静态站 + 公开链接
- [实时协作](/docs/realtime) — Liveblocks 多人实时编辑
- [输出格式](/docs/formats) — 文档/演示/表格模板与市场
- [分享与协作](/docs/sharing) — 协作者、文件分享、公共库、通知
